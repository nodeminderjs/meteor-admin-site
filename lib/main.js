...
Template.adminList.helpers({
  admListEachField: function(collectionName, options) {
    ...
    return eachField({
      ...
      customListFields: collectionObj.customListFields
    });
  },
  ...
  getContext: function(templateContext, collectionName) {
    console.log('***templateContext=', templateContext);
    console.log('***collectionName=', collectionName);
    console.log('***this=', this);
    var a = [];
    for (var k in this)
      a.push({name:k, value:this[k]});
    return {fields: a};
  }
});

Template.adminListRow.helpers({
  helper: function() {
    console.log('this=', this);
    return 'xxxxx';
  }
...
function eachField(params) {
  var fields = params.fields,
      ...
      customListFields = params.customListFields,
      customEditFields = params.customEditFields,
      ret = '', 
      first = true,
      field, value, label, path, sel, selOpts, editObj;

  for (var f in fields) {
    field = fields[f];
    sel = false;

    if (customListFields && customListFields[field]) {
      value = customListFields[field](doc[field]);
    } else {
      path = field.split('.');
      value = doc;
      while (path.length) {
    ...
    if (customEditFields && customEditFields[field]) {
      editObj = customEditFields[field];
      if (editObj.type == 'select') {
        sel = true;
        selOpts = editObj.options;
      }
      else 
